...
- **Контракт `data-testid` — инвариант.** Ни один существующий идентификатор не переименовывается и не исчезает. 71 статический плюс 8 шаблонных. Новые блоки получают новые идентификаторы.
...
Expected: PASS, в выводе `1 snapshot written`. Открыть снапшот и убедиться, что в нём **79** записей (71 статический + 8 шаблонных).
