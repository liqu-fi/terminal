...
`book.spread === null` — односторонняя книга: спреда не существует, и ноль вместо него читался бы как «нулевой спред». Долю печатать с двумя-тремя знаками (`spreadRatio` — WAD-доля; для процента с дробью использовать не `ratioPct`, а собственное деление на `10n ** 14n` с делением на 100 — округление до целого процента съело бы типичные 0,013%). Функцию для этого добавить в `bookView.ts` рядом с `ratioPct` и покрыть тестом в том же файле, что и остальные.
